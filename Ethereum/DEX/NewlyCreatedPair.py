...
#event_filter = contract.events.PairCreated.create_filter(fromBlock=17936073, toBlock=17936149)
        ...
        if len(events) > 0:
           ...
           for event in events:
               if (event['args']['token1'] == "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2"):
                   ...
                   print(f"Pair Created With Liquidity Amount {round(tokenLiquidityAmount,2)} ETH")

                   print("----------")

               elif (event['args']['token0'] == liquidityPairAddress):
                   tokenAddress1 = event['args']['token1']
                   getTokenName = web3.eth.contract(address=tokenAddress1,
                                                    abi=tokenNameABI)  # code to get name and symbol from token address
                   tokenName = getTokenName.functions.name().call()
                   tokenSymbol = getTokenName.functions.symbol().call()
                   tx_hash = web3.to_hex(event['transactionHash'])
                   lp_address= contract.functions.getPair(tokenAddress1,liquidityPairAddress).call()
                   lp_balance = get_liquidity_balance(lp_address)

                   tokenLiquidityAmount = float(web3.from_wei(lp_balance, "ether"))

                   print(
                       style.RED + getTimestamp() + " [Token] New Token Detected: " + style.CYAN + tokenName + " (" + tokenSymbol + "): " + style.MAGENTA + tokenAddress1 + "  https://etherscan.io/tx/" + tx_hash + style.RESET)
                   print(f"Pair Create With Liquidity Amount {round(tokenLiquidityAmount,2)} ETH")
