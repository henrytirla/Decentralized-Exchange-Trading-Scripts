...
import time
import json
import goplus_function
import websockets
#import buy_token
# from buy_token import Buy_Token
from alchemy import Alchemy, Network

from goplus_function import analyze_token
...
while True:
    try:
        ...
        for log in logs:
            ...
            if log.args.src == jareth_mev:  #log.args.src == jareth_mev or

                if log.args.dst not in processed_events:
                    #processed_events.add(tx_hash)
                    processed_events.add(log.args.dst)



                    print(f"{count} Transfer of {latest_block} {w3.from_wei(log.args.wad, 'ether')} WETH  from {log.args.src} to {log.args.dst} Tx_HASH:  https://etherscan.io/tx/{w3.to_hex(log.transactionHash)}")

                    token_address = get_token_from_lp(log.args.dst)


                    numberOfBuys, tokenName = getTotalBuys(jareth_mev,token_address)
                    print(style.GREEN + f"{numberOfBuys, tokenName}", style.RESET)
                     print("------------------")
