...
walletAddress ="0x776DD2229088fA48212E7e1c479F591451b90fC5"
if web3.isConnected():
    print(style.YELLOW + currentTimeStamp + " BSC TESTNET  Node successfully connected")
balance = web3.eth.getBalance("0x776DD2229088fA48212E7e1c479F591451b90fC5") # here balance is in wei
...
def updateTitle():
    ...
    sys.stdout.write( currentTimeStamp + "BSCTokenSniper  | Wallet Balance: " + str(walletBalance) + " BNB")
    sys.stdout.flush()
...
def Buy():
    ...
    for i in range(0,len(config.wallet_address)):
        ...
        pancakeswap2_txn = contractbuy.functions.swapExactETHForTokens(
            ...
            0,  # tokenB,
            [spend, tokenToBuy],
            config.wallet_address[i],
            (int(time.time()) + 10000)
        ).buildTransaction({

            'from': config.wallet_address[i],
            ...
            'nonce': web3.eth.get_transaction_count(config.wallet_address[i]),
        })
        # Sign transaction with priavte key here
        signed_txn = web3.eth.account.sign_transaction(pancakeswap2_txn, config.privatekeys[i])
        tx_token = web3.eth.send_raw_transaction(signed_txn.rawTransaction)
        print(currentTimeStamp,"Snipe was succesfull, bought: ", 'https://testnet.bscscan.com/tx/' + web3.toHex(tx_token))
